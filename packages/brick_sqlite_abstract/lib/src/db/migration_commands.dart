// Heavily, heavily inspired by [Aqueduct](https://github.com/stablekernel/aqueduct/blob/master/aqueduct/lib/src/db/schema/migration.dart)

export 'package:brick_sqlite_abstract/src/db/migration_commands/migration_command.dart';

export 'package:brick_sqlite_abstract/src/db/migration_commands/drop_column.dart';
export 'package:brick_sqlite_abstract/src/db/migration_commands/drop_table.dart';
export 'package:brick_sqlite_abstract/src/db/migration_commands/insert_column.dart';
export 'package:brick_sqlite_abstract/src/db/migration_commands/insert_foreign_key.dart';
export 'package:brick_sqlite_abstract/src/db/migration_commands/insert_table.dart';
export 'package:brick_sqlite_abstract/src/db/migration_commands/rename_column.dart';
export 'package:brick_sqlite_abstract/src/db/migration_commands/rename_table.dart';
