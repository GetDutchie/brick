...
Future<GraphqlRuntimeAssociationDefinition>
    _$GraphqlRuntimeAssociationDefinitionFromGraphql(Map<String, dynamic> data,
        {required GraphqlProvider provider,
        GraphqlFirstRepository? repository}) async {
  return GraphqlRuntimeAssociationDefinition(
      nonIterable: await AssocAdapter().fromGraphql(data['nonIterable'],
          provider: provider, repository: repository),
      iterable: await Future.wait<Assoc>(data['iterable']
              ?.map((d) => AssocAdapter()
...
Future<Map<String, dynamic>> _$GraphqlRuntimeAssociationDefinitionToGraphql(
    ...
    GraphqlFirstRepository? repository}) async {
  return {
    'nonIterable': await AssocAdapter().toGraphql(instance.nonIterable,
        provider: provider, repository: repository),
    'iterable': await Future.wait<Map<String, dynamic>>(instance.iterable
        .map((s) => AssocAdapter()
...
class GraphqlRuntimeAssociationDefinitionAdapter
  ...
  Future<GraphqlRuntimeAssociationDefinition> fromGraphql(
      ...
      await _$GraphqlRuntimeAssociationDefinitionFromGraphql(input,
          provider: provider, repository: repository);
  @override
  Future<Map<String, dynamic>> toGraphql(
          ...
          {required provider,
          covariant GraphqlFirstRepository? repository}) async =>
      await _$GraphqlRuntimeAssociationDefinitionToGraphql(input,
          provider: provider, repository: repository);
}
''';
